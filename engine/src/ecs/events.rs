...
use super::{
    ...
    entities::{ComponentStorage, Entities}
};
...
impl Events {
    pub fn new(entities: &mut Entities) -> Self {
    ...
    fn handle_mouse_events(&mut self, input: &Input) -> Vec<Event> {
        ...
        for mouse in input.pressed_mouse.iter() {
            match mouse {
                sdl3::mouse::MouseButton::Left => events.push(
    ...
    pub fn handle_input_events(&mut self, input: &Input) -> Vec<Event> {
