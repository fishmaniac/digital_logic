...
where
    ...
    Entity: GameComponent,
{
    pub fn new(entity_id: u32, state: State, transition_callback: TransitionFn<State, Entity>) -> Self {
        StateMachine {
            entity_id,
            state,
            transition_callback,
    ...
    fn listener(entities: &mut Entities, entity_id: u32, event: &events::Event) {
        match event {
            events::Event::StateUpdate => {
                ...
                let game_component: &mut Entity = match entities::get_game_component_mut(components, entity_id) {
                    Some(game_component) => game_component,
                    None => return println!("No state machine component found for id: {}", entity_id),
                };
                ...
                let state = match Self::get_state_machine(state_components, entity_id)
                    ...
                    None => return println!("No state machine found for id: {}", entity_id),
                };
            },
            _ => {},
        }
...
where
        Self: Sized {
        components.get_mut(entity_id as usize).and_then(|state| state.as_mut())
    }
    fn as_any_mut(&mut self) -> &mut dyn Any {
        self
    }
